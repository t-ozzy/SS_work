...
h_img_1 = cv2.cvtColor(img_1, cv2.COLOR_BGR2HSV)
h_img_2 = cv2.cvtColor(img_2, cv2.COLOR_BGR2HSV)
# hsvのsとvを全部127にしたい
# h_img_1[:, :, 1] = 127
# h_img_1[:, :, 2] = 127
# h_img_2[:, :, 1] = 127
...
s_img_1 = cv2.cvtColor(img_1, cv2.COLOR_BGR2HSV)
...
s_img_2[:, :, 1] = 127
if DEBUG:
    cv2.imwrite("s_1.png", s_img_1)
    cv2.imwrite("s_2.png", s_img_2)


v_img_1 = cv2.cvtColor(img_1, cv2.COLOR_BGR2HSV)
v_img_2 = cv2.cvtColor(img_2, cv2.COLOR_BGR2HSV)
# hsvのsとvを全部127にしたい
# v_img_1[:, :, 1] = 127
# v_img_1[:, :, 0] = 127
# v_img_2[:, :, 1] = 127
...
v_img_2[:, :, 2] = 127
if DEBUG:
    cv2.imwrite("v_1.png", v_img_1)
    cv2.imwrite("v_2.png", v_img_2)

# # グレースケール変換
# gray_1 = cv2.cvtColor(hsv_img_1, cv2.COLOR_HSV2GRAY)
# gray_2 = cv2.cvtColor(hsv_img_2, cv2.COLOR_HSV2GRAY)
# if DEBUG:
#     cv2.imwrite("gray_1.png", gray_1)
#     cv2.imwrite("gray_2.png", gray_2)
...
# h_img_1 = clahe.apply(h_img_1)
# h_img_2 = clahe.apply(h_img_2)
# # h_img_1 = cv2.equalizeHist(h_img_1)
# # h_img_2 = cv2.equalizeHist(h_img_2)
# if DEBUG:
#     cv2.imwrite("clahe_h_1.png", h_img_1)
#     cv2.imwrite("clahe_h_2.png", h_img_2)
# s_img_1 = clahe.apply(s_img_1)
# s_img_2 = clahe.apply(s_img_2)
# # s_img_1 = cv2.equalizeHist(s_img_1)
# # s_img_2 = cv2.equalizeHist(s_img_2)
# if DEBUG:
#     cv2.imwrite("clahe_s_1.png", s_img_1)
#     cv2.imwrite("clahe_s_2.png", s_img_2)
# v_img_1 = clahe.apply(v_img_1)
# v_img_2 = clahe.apply(v_img_2)
# # v_img_1 = cv2.equalizeHist(v_img_1)
# # v_img_2 = cv2.equalizeHist(v_img_2)
# if DEBUG:
#     cv2.imwrite("clahe_v_1.png", v_img_1)
#     cv2.imwrite("clahe_v_2.png", v_img_2)

# 画像を引き算
h_diff = cv2.absdiff(h_img_1, h_img_2)
if DEBUG:
    cv2.imwrite("h_diff.png", h_diff)
s_diff = cv2.absdiff(s_img_1, s_img_2)
if DEBUG:
    cv2.imwrite("s_diff.png", s_diff)
v_diff = cv2.absdiff(v_img_1, v_img_2)
if DEBUG:
    cv2.imwrite("v_diff.png", v_diff)


_, h_th = cv2.threshold(h_diff, 255 // 4, 255, cv2.THRESH_BINARY)  # 閾値40は手作業
if DEBUG:
    cv2.imwrite("h_th.png", h_th)
_, s_th = cv2.threshold(s_diff, 255 // 4, 255, cv2.THRESH_BINARY)  # 閾値40は手作業
if DEBUG:
    cv2.imwrite("s_th.png", s_th)
_, v_th = cv2.threshold(v_diff, 255 // 4, 255, cv2.THRESH_BINARY)  # 閾値40は手作業
if DEBUG:
    cv2.imwrite("v_th.png", v_th)
# 論理和で結合
img_th = cv2.bitwise_or(h_th, s_th)
img_th = cv2.bitwise_or(img_th, v_th)
if DEBUG:
...
img_th = cv2.adaptiveThreshold(
    img_th, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2
)
if DEBUG:
    cv2.imwrite("th.png", img_th)
